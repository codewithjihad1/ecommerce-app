            ...
            <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
                        ...
                        <View className="flex-row items-center flex-1">
                            ...
                            <View className="w-16 h-16 rounded-full bg-pink-200 items-center justify-center overflow-hidden">
                                {user.user_metadata?.avatar ? (
                                    <Image source={{ uri: user.user_metadata?.avatar }} className="w-full h-full" />
                                ) : (
                                    <Text className="text-4xl font-bold">{user.user_metadata.name?.charAt(0)}</Text>
                                )}
                            </View>

                            {/* User Info */}
                            <View className="ml-4 flex-1">
                                <Text className="text-lg font-semibold text-gray-900">{user?.user_metadata?.name}</Text>
                                <Text className="text-sm text-gray-500 mt-0.5">{user?.user_metadata?.email}</Text>
                            </View>
                        </View>
                    ...
                    {menuItems.map((item, index) => {
                        ...
                        return (
                            <TouchableOpacity
                                ...
                                <Text className="flex-1 ml-4 text-base text-gray-700">{item.label}</Text>
                ...
                <TouchableOpacity
                    ...
                    <View className="flex-row items-center px-5 py-4">
                        ...
                        <Text className="flex-1 ml-4 text-base text-gray-700">Log out</Text>
                    </View>
                </TouchableOpacity>
            </ScrollView>
        </SafeAreaView>
